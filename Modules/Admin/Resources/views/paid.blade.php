...
<table id="example2" class="table">
    ...
    <tbody>
        <?php
        if (isset($paid_info['row_count']) && $paid_info['row_count'] > 0) {
            foreach ($paid_info['data'] as $value) {
                $booking_info = getBookingInfo($value->id);
        ?>
                <tr data-id="{{ my_encrypt($value->id) }}" data-url="{{ route('booking-management-info',['ready_to_pay' => 2]) }}">
