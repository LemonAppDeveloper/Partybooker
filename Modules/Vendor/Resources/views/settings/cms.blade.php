            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="pheading" style="margin-bottom: 20px;">
                        <h2><a href="{{ route('vender.configration') }}"><i class="las la-arrow-left"></i></a> Pages Management</h2>
                    </div>
                </div>
                <div class="col-md-4 text-end d-none">
                    <button class="btn btn-secondary btn-add">Add New</button>
                </div>
                <div class="clearfix"></div>
                <div class="col-md-8">
                    <div class="order-conf">
                        <h3 class="title">Update Template</h3>
                        <form action="{{ route('vender.configration.cms') }}" name="form-add" onsubmit="return false;">
                            @csrf
                ...
                <div class="col-md-4">
                    <div class="order-list">
                        ...
                        <div class="cms-list">
                            <?php
                            echo view('vendor::settings.cms-list', compact('data'));
                            ?>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('[name="form-add"]').submit(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        get_cms_listing($('[name="form-add"]').find('[name="id"]').val());
        ...
        function get_cms_listing(id) {
            $('#cover-spin').show();
            $.ajax({
                url:  '/vendor/configration/cms-list',
                type: 'GET',
                data: {
                    'action': 'list',
                    'id': id
                ...
                success: function(response) {
                    $('#cover-spin').hide();
                    if (response.status) {
                        $('.cms-list').html(response.data.html);
                    } else {
                        if (typeof response.message == 'object') {
                            $.each(response.message, function(index, message) {
                                $('<p class="text-danger custom-validation-message">' + message + '</p>').insertAfter('[name="' + input + '"]');
